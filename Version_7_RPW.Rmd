...
============
We will perform a cost-benefit analysis assessing a set of conservation policies put into place to protect susceptible species from the RPW. Based on the policies selected, two species appear: canary and date palms [@generalitatdecatalunyaOrdreARP3432006]. The former was reported in 99.69% and the latter in 0.23% of RPW infestation cases in Catalonia [@generalitatdecatalunyaPrevencioLluitaContra2011]. Although date palms currently represent a small fraction of reports, we include them because in absence of canary palms, the RPW would likely shift to infesting date palms. 
...
We modelled the evolution of tree stocks as a system dynamics problem since this approach allows us to understand nonlinear behavior over time using stocks, flows and feedback loops [@forresterBeginningSystemDynamics1989]. The two palm species considered in this study have similar processes with different initialisation values so we implemented two separate modules of the same model. From this point onward, susceptible palm species will be referred to as *tree*. The model was designed with three stocks: healthy, infested and treated trees. Figure \ref{fig:model} illustrates the model using Vensim software [add citation].

\begin{figure}[!htb]
\centerline{\includegraphics[width=30pc]{model.png}} 
\caption{The model has three main stocks: healthy, infested, and treated trees. Each stock hsa ingoing and outgoing flows that depend on external and internal mechanics. For example, healthy trees depends on the replanting flow depends on external decisions. In contrast, the number of infested trees is part of a positive feedback loop where more infested trees lead to a larger pool of infested trees.}
\label{fig:model}
\end{figure}
...
Healthy trees, $S_t^H$, are all susceptible trees with two incoming flows: replanted trees, $F_{t,t-1}^R$, and successfully treated trees, $F_{t,t-1}^{ST}$. Prevention consists on a series of chemical applications to reduce tree vulnerability to the pest. Outgoing flows are trees in which prevention has failed, $F_{t,t-1}^{FP}$. Therefore, the equation for the healthy net flow, $S_t^H-S_{t-1}^H$, between $t-1$ and $t$ is:
...
Treated trees, $F_{t,t-1}^{TR}$, is an incoming flow to treated trees, $S_t^T$. The treated trees stock has outgoing flows representing either failed, $F_{t,t-1}^{FT}$, or successful treatment, $F_{t,t-1}^{ST}$. Thus,
...
\begin{equation} 
\label{eq: lost_stock}S_t^L-S_{t-1}^L=F_{t,t-1}^{UTR}+F_{t,t-1}^{FT}
\end{equation}
...
\begin{equation} 
\label{eq: replanted_stock}S_t^R-S_{t-1}^R=F_{t,t-1}^R
\end{equation}

Costs
=======

We now introduce equations to calculate costs. These are divided into two main groups. The first is management costs assumed by municipalities and private palm owners from 2011 onwards. These management costs, $M_t$, for trees in year $t$ are described as the sum of inspection, $C_t^{inspect}$, prevention, $C_t^{prevent}$, treatment costs, $C_t^{treat}$, replanting, $C_t^{replant}$, and removal costs, $C_t^{remove}$. The second group is government (GENCAT) costs which includes management costs (except replanting) but also additional activities such as research and outreach. These were provided from 2005 to 2017 and since no specific breakdown was provided by GENCAT so they are added as a lump sum ($GENCAT_t$).

\begin{equation}
\label{eq:management_costs}M_t=C_t^{inspect}+C_t^{prevent}+C_t^{treat}+C_t^{replant}+C_t^{removal}+GENCAT_t
\end{equation} 
...
Here, $S_t^H$, is the number of healthy trees ($H$) in year $t$ and $c^{inspect}$ is the inspection unit cost. Prevention costs, $C_t^{prevent}$, are the costs of applying preventative treatments to healthy trees in year $t$.
...
\begin{equation}
\label{eq:treatment_costs}C_t^{treat}=S_t^T*c^{treat}
\end{equation} 									
...
\begin{equation} 
\label{eq:replanting_costs}C_t^{replant}=S_t^R*c^{replant}
\end{equation} 	

Here, $S_t^R$, is the number of replanted trees ($R$) in year $t$ and $c^{replant}$ is the unit cost of replanting a new tree. 

$C_t^{remove}$, corresponds to the costs of permanently removing a tree in year $t$, which the pest has damaged beyond the point of treatment, $S_t^L$.

\begin{equation} 
\label{eq:removal_costs}C_t^{remove}=S_t^L*c^{remove}
\end{equation} 						     	
...
carbon is an economic indicator that represents the net impacts from the global climate change that results from a one tonne increase in carbon dioxide emissions [@iawgTechnicalUpdateSocial2016].

\begin{equation} 
\label{eq:social_cost}VCS=SCC*c*S_t 
\end{equation}

Where, $SCC$ is the social cost of carbon dioxide, $c$ is the amount of carbon sequestered by a tree; and $S_t$ is the total tree stock in year $t$.

Benefits of tree adjacency can be captured by using the hedonic property-value method. This method helps estimate the aesthetic value trees contribute to property sale prices. Several studies have focused on quantifying forest adjacency [@donovanTreesCityValuing2010; @sanderValueUrbanTree2010; @tyrvainenPropertyPricesUrban2000] with results stating that trees contribute between 0.29 to 5% of property values of private residences.
...
Here, $gamma$, is the percentage variation in property values associated to forestry adjacency and $S_{2005}$ and $S_{2017}$ are the estimated number of palm trees in 2005 and 2017, respectively. The difference in a property's value $PV$ between 2005 and 2017 is derived from three changes: market price ($MP$), inflation ($I$), and palm tree adjacency ($PA$).

\begin{equation} 
\label{eq:pv_2017}PV_{2017}-PV_{2005}=PV_{2005}*(\%MP+\%I+\%PA)
\end{equation}

Here, $\%MP$, is the percentage change in property value, $\%I$, is the percentage change in inflation, and $\%PA$, is the percentage change is palm tree adjacency between 2017 and 2005. Solving for $PV_{2005}$ yields Equation \ref{eq:pv_2016}.

\begin{equation} 
\label{eq:pv_2016}PV_{2005}=\frac{PV_{2017}}{1+\%MP+\%I+\%PA}
\end{equation}

If $\%MP$ and $\%I$ are zero, the property value loss associated to RPW impact on palm tree adjacency in Catalonia between 2017 and 2005 is summarised in Equation \ref{eq:pv_diff}.
...
The net social benefits (NSB) of the current management policy against the RPW in Catalonia is the difference between benefits and costs, both brought to present value (Eq. \ref{eq:nsb}).
...
As explained earlier, the model has three parameters: effective contact rate ($C_E$), probability of being treated ($tau$), and probability of successful treatment ($sigma$). In this section, we explain the
process to estimate each parameter.

\subsubsection{Effective contact rate ($C_E$)}

Effective contact rate, $C_E$, is defined as the sufficient contact rate to lead to transmission if it occurs between an infested and a susceptible tree [@vynnyckyIntroductionInfectiousDisease2010] with units $frac{palms}{(palm/year)}$. Palm trees in Catalonia are often planted in a single file, thus a single tree can have two neighboring trees. $C_E$ can take up values [1,$infty$] to which we assigned [1,2] since a single infested tree can infest up to two trees.

\subsubsection{Probability of being treated ($\tau$)}

With the goal of eliciting treatment and treatment success
probabilities, we conducted interviews with three government officials and two local independent firms offering pest management services (healthy tree inspection and prevention, and infested tree treatment and removal). Tau ($\tau$) has been defined as the probability that an infested tree is treated. $\tau$ can take values between [0,1] to which we assigned the interval [0.5,1]. Since legislation states that treatment of all affected trees is mandatory, we assigned 1 to the upper boundary for the case in which 100% of infested trees are treated. According to interviewees, there are instances where infested tree treatment does not occur. This may happen deliberately or accidentally. Furthermore, no official data exists describing the behaviour of private owners while municipality reports are compromised since they must reflect compliance with the law. In order to account for this unknown behaviour where treatment does not occurr but is unknown, the lower boundary was set to a probability of 0.5 (i.e.~coin toss).

\subsubsection{Probability of successful treatment ($\sigma$)}

Sigma ($\sigma$) is the probability that treatment is successful. Again, as it is a probability, it can take values between [0,1]. Success is defined as a tree being sufficiently recovered to rejoin the healthy stock. Interviewees were consistent in their responses stating that once a tree becomes infested, survival probability is slim to none. For this reason, we assigned [0,0.5] to the interval as it reflects the pessimistic outlook on trees under treatment: from a zero recovery rate up to 0.5 (i.e.~coin toss).

2.4 Model calibration
========================

We implemented the model using the deSolve package in R Software [@soetaertSolvingDifferentialEquations2010]. To run the model, two initial stock values had to be defined for $t=2005$, healthy, $S_{2005}^H$, and infested, $S_{2005}^I$, stock. In this section we explain how these two initialisation values were set. 
...
During the model selection stage, we considered several estimators: best predictive model (BPM), median probability model (MPM) and, highest probability model (HPM). In order to select amongst estimators, we computed the average predictive error (APE) for each one. BPM, the estimator that has predictions closest to the BMA and corresponding posterior standard deviations, was selected for date palms and HPM, the model with the lowest $BIC$ was selected for canary palms. In terms of coefficients this meant, distance to the coastline, population, fraction of GDP from agriculture and industry, for date palms; and distance to the coastline, population, and fraction of GDP from industry, for canary palms.
...
color = cut(ser, breaks = 4),
  ...
  guides(
    size = guide_legend(title = "Population"),
      color=guide_legend(title = "Fraction of GDP from services")
  ) +
  labs(
...
Using these models, we estimated the number of date and canary palms in Catalonia. Estimations appear in Figure \ref{fig:predictions} as a function of distance, population, and fraction of GDP for industry. We can see that for most municipalities, palm trees are located closest to the coast, have a positive relationship with population and negative relationship with fraction of GDP from industry. If we consider industry to be opposed to services, it is likely this variable serves as a proxy for tourism.

\begin{figure}[!htb] 
\centerline{\includegraphics[width=30pc]{predictions.png}} 
\caption{Palm tree estimations according to species in 2017. The upper plot shows date species estimations and the lower, canary. Both estimations are plotted against three variables: distance to the coastline, population, and fraction of GDP from services. Overall, there are more date than canary trees. In both cases distance to the coastline has an inverse relationship with palm occurence. Population and fraction of GDP from services both have a positive effect on the number of palm trees. This matches our intuition since the most frequent use for palm trees in Catalonia is ornamental. They are often found along tourist areas, and tourism contributes to the service economy.}
...
In this section we will present calculations for costs, benefits, and the related cost-benefit analysis (Eq. \ref{eq:nsb}). In this section, tables and figures show aggregated results, for detailed tables and figures refer to Appendix C. 
...
We considered two types of benefits provided by palm trees (Eq. \ref{eq:benefits}), those derived from carbon sequestration and others derived from palm tree adjacency. Here we present the results for both calculations. 
...
The dataset contained 2118 observations describing detached properties on sale in Catalonia during October 2018. The properties were distributed evenly amongst Catalonia with at least 50 observations per
*comarca*, which we defined earlier as our strata. Upon initial exploration of the dataset, there were 8 numerical and 21 character variables.

The first step was to identify categorical variables within all the character variables. Here, some were identified as categorical and others related to the unique property address were left as character.
The second step was to deal with missingness in the data. Most missing values appeared in variables that signalled the presence or absence of a feature (i.e.~swimming pool, wardrobe, terrace, garden, etc.). Since these missing values were a case of missing completely at random (MCAR), it was safe to remove observations without introducing bias into the
data. However, we wanted to keep as many observations as possible so we performed imputation. After the dataset was complete, we transformed all categorical variables into numerical ones. Ordinal categorical variables were transformed into a scale, and dummy variables were created for non-ordinal ones. After this process, the full dataset had 34 variables.
...
Again using the ggmap package for R software [@kahleGgmapSpatialVisualization2013] and each unique property address, individual images were printed. Each image was screened for either date or canary palms. After removing duplicate properties and addresses which were not found on Google, the dataset had 1714 observations with 27 variables.
...
all_models <- dredge(chosen_fit, rank = "BIC")
#Save models
...
```{r numeric variables plot, include=FALSE}
...
```{r categorical variables plot, include=FALSE}
#Save column names
cols <- c("BATHNUMBER", 
                     "HASGARDEN", 
                     "HASSWIMMINGPOOL",
...
The model that minimises the BIC include number of bathrooms, constructed area, constructed year, garden, swimming pool, wardrobe and size of the plot of land as regressors.
...
-   2007/365/CE: On 25th May 2007, the EU Commission publishes a decision based on the RPW spread along the Iberian Peninsula. The decision specifies palm tree vulnerability and announces a series of measures to be taken by member countries, including delimitation of areas at RPW risk [@eucomission2007365CE2007].

-   2008/776/CE: On 6th October 2008, the previous decision 2007/365/CE is modified by expanding the range of vulnerable palm trees [@eucomission2008776CE2008].
